...
DEFAULT_EVAL_MODEL = os.environ.get("LOCAL_EVAL_MODEL_NAME", "gpt-4o-mini")
MAX_API_RETRIES = 3
...
DEFAULT_TOKENIZER_PATH = "meta-llama/Llama-3.2-1B-Instruct"
# Check if running locally to use custom paths
RUN_LOCAL = os.environ.get("RUN_LOCAL", "False").lower() in ("true", "1", "t")
LOCAL_TOKENIZER_PATH = os.environ.get("LOCAL_TOKENIZER_PATH", DEFAULT_TOKENIZER_PATH)
...
class CRAGEvaluator:
    ...
    def __init__(
        ...
        # Use local tokenizer path if running locally
        tokenizer_path = LOCAL_TOKENIZER_PATH if RUN_LOCAL else DEFAULT_TOKENIZER_PATH
        console.print(f"[blue]Using tokenizer from: {tokenizer_path}[/blue]")
        #self.tokenizer = Tokenizer.from_pretrained(tokenizer_path,local_files_only=True)
...
def main() -> None:
    ...
    dataset_path = os.environ.get("EVAL_DATASET_PATH",repo_name)
    console.print(
        f"[bold green]Loading from HuggingFace:[/bold green] {repo_name} (revision: {args.revision})"
    )
    dataset = load_dataset(dataset_path, revision=args.revision)
    ...
    # Check for local search API paths
    if RUN_LOCAL:
        search_api_text_model_name = os.environ.get("LOCAL_TEXT_MODEL_PATH", search_api_text_model_name)
        search_api_image_model_name = os.environ.get("LOCAL_IMAGE_MODEL_PATH", search_api_image_model_name)
        search_api_web_hf_dataset_id = os.environ.get("LOCAL_WEB_DATASET_ID", search_api_web_hf_dataset_id)
        search_api_image_hf_dataset_id = os.environ.get("LOCAL_IMAGE_DATASET_ID", search_api_image_hf_dataset_id)
    ...
    evaluator = CRAGEvaluator(
        ...
        agent=UserAgent(search_pipeline=search_pipeline,model_name=model_path),
